...
function parseClientUrls(value) {
  ...
  const hasPlaceholder = rawUrls.some((url) => url.includes('your-frontend-url.com'));
  const usableUrls = hasPlaceholder ? [] : rawUrls;
  const shouldIncludeLocalDefaults = (process.env.NODE_ENV || 'development') !== 'production';
...
export const env = {
  ...
  firebasePrivateKey: parseMultilineSecret(process.env.FIREBASE_PRIVATE_KEY),
  razorpayKeyId: process.env.RAZORPAY_KEY_ID || '',
  razorpayKeySecret: process.env.RAZORPAY_SECRET || process.env.RAZORPAY_KEY_SECRET || '',
  hasFirebaseAdminConfig,
  useMockStore: !hasFirebaseAdminConfig,
  allowDevAuthBypass:
    (process.env.NODE_ENV || 'development') !== 'production' && !hasFirebaseAdminConfig,
};
...
export function validateEnvironment() {
  if (env.nodeEnv === 'production') {
    ...
    if (!env.imgbbApiKey) missing.push('IMGBB_API_KEY');
